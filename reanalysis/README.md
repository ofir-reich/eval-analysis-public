...
- **Caches:** the committed `data/` CSVs double as caches for the expensive steps (the bootstrap replicates in `xboot`, the SIMEX curves) — delete a cache file to force a recompute; seeds are fixed, so a recompute reproduces it exactly. `N_BOOT` and `N_SIMEX` env vars trade runtime for precision.
